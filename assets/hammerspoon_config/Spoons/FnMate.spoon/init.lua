--- === FnMate ===
---
--- Use Fn + `h/l/j/k` as arrow keys, `y/u/i/o` as mouse wheel, `,/.` as left/right click, Fn + '0' for Cmd-Left, Fn + '4' for Cmd-Right
---
...
function obj:init()
    local function catcher(event)
        ...
        elseif event:getFlags()['fn'] and event:getCharacters() == "0" then
            return true, {hs.eventtap.event.newKeyEvent({"command"}, "left", true)}
        elseif event:getFlags()['fn'] and event:getCharacters() == "4" then
            return true, {hs.eventtap.event.newKeyEvent({"command"}, "right", true)}
